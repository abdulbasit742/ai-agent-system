#!/usr/bin/env python3
"""Create and verify portable Merkle checkpoints for audit trust receiver states."""
...
CHECKPOINT_FIELDS = {
    "checkpoint_version", "state_id", "entry_count", "head", "merkle", "checkpoint_id",
}
CHECKPOINT_REFERENCE_FIELDS = {
    "checkpoint_id", "state_id", "entry_count", "merkle_root",
}
PROOF_FIELDS = {"proof_version", "checkpoint", "entry", "audit_path", "proof_id"}
MERKLE_FIELDS = {"algorithm", "root"}


class AuditTrustReceiverCheckpointError(ValueError):
    """Raised when a receiver checkpoint, proof, lineage, or pin is invalid."""
...
def _exact(value: Any, fields: set[str], label: str) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != fields:
        raise AuditTrustReceiverCheckpointError(
            f"{label} fields do not match the reviewed schema", rule_id="ARC002"
        )
    return value
...
def _hash(value: Any, label: str) -> str:
    if not isinstance(value, str) or not HEX_64.fullmatch(value):
        raise AuditTrustReceiverCheckpointError(
            f"{label} must be 64 lowercase hexadecimal characters", rule_id="ARC002"
        )
    return value
...
def _pin(value: Any, label: str) -> str:
    if not isinstance(value, str):
        raise AuditTrustReceiverCheckpointError(
            f"{label} must be a string", rule_id="ARC003"
        )
    lowered = value.lower()
    if not HEX_64.fullmatch(lowered):
        raise AuditTrustReceiverCheckpointError(
            f"{label} must be 64 hexadecimal characters", rule_id="ARC003"
        )
    return lowered
...
def _checkpoint_identifier(payload: dict[str, Any]) -> str:
    return hashlib.sha256(
        b"audit-trust-receiver-checkpoint-v1\x00" + canonical_json(payload)
    ).hexdigest()


def _proof_identifier(payload: dict[str, Any]) -> str:
    return hashlib.sha256(
        b"audit-trust-receiver-inclusion-proof-v1\x00" + canonical_json(payload)
    ).hexdigest()


def _lineage_identifier(payload: dict[str, Any]) -> str:
    return hashlib.sha256(
        b"audit-trust-receiver-lineage-v1\x00" + canonical_json(payload)
    ).hexdigest()


def _head(value: Any) -> dict[str, Any]:
    raw = _exact(value, HEAD_FIELDS, "receiver checkpoint head")
    return {
        "sequence": _integer(raw["sequence"], "receiver head sequence", 1),
        "entry_hash": _hash(raw["entry_hash"], "receiver head entry hash"),
        "handoff_bundle_id": _hash(raw["handoff_bundle_id"], "receiver head handoff id"),
        "checkpoint_id": _hash(raw["checkpoint_id"], "receiver head trust checkpoint id"),
        "state_id": _hash(raw["state_id"], "receiver head trust state id"),
        "entry_count": _integer(raw["entry_count"], "receiver head trust entry count", 1),
        "head_bundle_id": _hash(raw["head_bundle_id"], "receiver head trusted bundle id"),
        "generation": _integer(raw["generation"], "receiver head generation", 1),
        "segment_count": _integer(raw["segment_count"], "receiver head segment count", 1),
    }


def _entry(value: Any) -> dict[str, Any]:
    raw = _exact(value, ENTRY_FIELDS, "receiver proof entry")
    if raw["entry_version"] != ENTRY_VERSION:
        raise AuditTrustReceiverCheckpointError("receiver proof entry version is unsupported")
    sequence = _integer(raw["sequence"], "receiver proof sequence", 1)
    kind = raw["kind"]
    if kind not in {"anchor", "transition"} or (sequence == 1) != (kind == "anchor"):
        raise AuditTrustReceiverCheckpointError(
            "receiver proof entry kind is inconsistent with sequence"
        )
    previous_entry_hash = _hash(raw["previous_entry_hash"], "receiver proof previous hash")
    if sequence == 1 and previous_entry_hash != ZERO_HASH:
        raise AuditTrustReceiverCheckpointError(
            "receiver anchor proof entry must use the zero previous hash"
        )
    try:
        evidence = _evidence(_exact(raw["evidence"], EVIDENCE_FIELDS, "receiver proof evidence"))
        admission = _admission(
            _exact(raw["admission"], ADMISSION_FIELDS, "receiver proof admission")
        )
        if sequence == 1:
            if raw["transition"] is not None:
                raise AuditTrustReceiverCheckpointError(
                    "receiver anchor proof entry must not contain transition evidence"
                )
            transition = None
        else:
            transition = _transition(
                _exact(raw["transition"], TRANSITION_FIELDS, "receiver proof transition")
            )
    except AuditTrustReceiverError as exc:
        raise AuditTrustReceiverCheckpointError(str(exc), rule_id="ARC002") from exc
    core = _entry_payload(
        sequence, kind, previous_entry_hash, evidence, admission, transition
    )
    entry_hash = _hash(raw["entry_hash"], "receiver proof entry hash")
    expected = receiver_identifier(b"audit-trust-receiver-entry-v1", core)
    if entry_hash != expected:
        raise AuditTrustReceiverCheckpointError(
            "receiver proof entry hash does not match its canonical payload",
            rule_id="ARC006",
        )
...
def _largest_power_of_two_less_than(size: int) -> int:
    if size < 2:
        raise AuditTrustReceiverCheckpointError("Merkle split requires at least two leaves")
    return 1 << ((size - 1).bit_length() - 1)


def _merkle_root(leaves: list[bytes]) -> bytes:
    if not leaves:
        raise AuditTrustReceiverCheckpointError("Merkle tree requires at least one leaf")
    if len(leaves) == 1:
        return leaves[0]
...
def _audit_path(leaves: list[bytes], index: int) -> list[bytes]:
    if index < 0 or index >= len(leaves):
        raise AuditTrustReceiverCheckpointError("proof index is outside the Merkle tree")
    if len(leaves) == 1:
        return []
    split = _largest_power_of_two_less_than(len(leaves))
    if index < split:
...
def _rebuild_root(
    ...
    if index < split:
        ...
        if position[0] >= len(siblings):
            raise AuditTrustReceiverCheckpointError(
                "receiver inclusion proof audit path is too short", rule_id="ARC006"
            )
...
def _verify_inclusion(
    ...
    if sequence < 1 or sequence > size:
        raise AuditTrustReceiverCheckpointError(
            "receiver proof sequence is outside the checkpoint entry range",
            rule_id="ARC006",
        )
    siblings = [bytes.fromhex(_hash(item, "receiver proof path hash")) for item in audit_path]
    position = [0]
    root = _rebuild_root(_leaf_hash(entry), sequence - 1, size, siblings, position)
    if position[0] != len(siblings):
        raise AuditTrustReceiverCheckpointError(
            "receiver inclusion proof audit path contains extra hashes",
            rule_id="ARC006",
        )
    return root.hex()
...
def create_checkpoint(state: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "merkle": {"algorithm": MERKLE_ALGORITHM, "root": _merkle_root(leaves).hex()},
    }
    return {**payload, "checkpoint_id": _checkpoint_identifier(payload)}


def validate_checkpoint(value: Any) -> dict[str, Any]:
    raw = _exact(value, CHECKPOINT_FIELDS, "receiver checkpoint")
    if raw["checkpoint_version"] != CHECKPOINT_VERSION:
        raise AuditTrustReceiverCheckpointError(
            f"receiver checkpoint version must be {CHECKPOINT_VERSION}"
        )
    state_id = _hash(raw["state_id"], "receiver checkpoint state id")
    entry_count = _integer(raw["entry_count"], "receiver checkpoint entry count", 1)
    head = _head(raw["head"])
    if head["sequence"] != entry_count:
        raise AuditTrustReceiverCheckpointError(
            "receiver checkpoint head sequence differs from entry count",
            rule_id="ARC002",
        )
    merkle_raw = _exact(raw["merkle"], MERKLE_FIELDS, "receiver checkpoint Merkle data")
    if merkle_raw["algorithm"] != MERKLE_ALGORITHM:
        raise AuditTrustReceiverCheckpointError(
            "receiver checkpoint Merkle algorithm is unsupported"
        )
    merkle = {
        "algorithm": MERKLE_ALGORITHM,
        "root": _hash(merkle_raw["root"], "receiver checkpoint Merkle root"),
    }
    core = {
        "checkpoint_version": CHECKPOINT_VERSION,
        "state_id": state_id,
    ...
    checkpoint_id = _hash(raw["checkpoint_id"], "receiver checkpoint id")
    if checkpoint_id != _checkpoint_identifier(core):
        raise AuditTrustReceiverCheckpointError(
            "receiver checkpoint id does not match its canonical payload",
            rule_id="ARC002",
        )
...
def checkpoint_matches_state(
    ...
    if normalized_checkpoint != expected:
        raise AuditTrustReceiverCheckpointError(
            "checkpoint does not match the canonical receiver state",
            rule_id="ARC004",
        )
...
def create_proof(
    ...
    *,
    ...
    if (sequence is None) == (handoff_bundle_id is None):
        raise AuditTrustReceiverCheckpointError(
            "select exactly one receiver entry by sequence or handoff bundle id",
            rule_id="ARC005",
        )
    selected: dict[str, Any] | None = None
    if sequence is not None:
        _integer(sequence, "receiver proof sequence", 1)
        if sequence <= len(normalized_state["entries"]):
            selected = normalized_state["entries"][sequence - 1]
    else:
        wanted = _pin(handoff_bundle_id, "receiver proof handoff bundle id")
        for entry in normalized_state["entries"]:
            if entry["evidence"]["handoff_bundle_id"] == wanted:
                selected = entry
                break
    if selected is None:
        raise AuditTrustReceiverCheckpointError(
            "requested handoff entry is not present in receiver state",
            rule_id="ARC005",
        )
...
def validate_proof(value: Any) -> dict[str, Any]:
    raw = _exact(value, PROOF_FIELDS, "receiver inclusion proof")
    if raw["proof_version"] != PROOF_VERSION:
        raise AuditTrustReceiverCheckpointError(
            f"receiver inclusion proof version must be {PROOF_VERSION}"
        )
    reference_raw = _exact(
        raw["checkpoint"], CHECKPOINT_REFERENCE_FIELDS, "receiver proof checkpoint reference"
    )
    reference = {
        "checkpoint_id": _hash(reference_raw["checkpoint_id"], "receiver proof checkpoint id"),
        "state_id": _hash(reference_raw["state_id"], "receiver proof state id"),
        "entry_count": _integer(reference_raw["entry_count"], "receiver proof entry count", 1),
        "merkle_root": _hash(reference_raw["merkle_root"], "receiver proof Merkle root"),
    }
    ...
    if not isinstance(audit_path_raw, list) or len(audit_path_raw) > MAX_AUDIT_PATH:
        raise AuditTrustReceiverCheckpointError(
            "receiver proof audit path is malformed or exceeds the reviewed limit",
            rule_id="ARC006",
        )
    audit_path = [_hash(item, "receiver proof path hash") for item in audit_path_raw]
    rebuilt = _verify_inclusion(
        entry, entry["sequence"], reference["entry_count"], audit_path
    )
    if rebuilt != reference["merkle_root"]:
        raise AuditTrustReceiverCheckpointError(
            "receiver proof does not reconstruct the checkpoint Merkle root",
            rule_id="ARC006",
        )
    core = {
        "proof_version": PROOF_VERSION,
    ...
    proof_id = _hash(raw["proof_id"], "receiver proof id")
    if proof_id != _proof_identifier(core):
        raise AuditTrustReceiverCheckpointError(
            "receiver proof id does not match its canonical payload",
            rule_id="ARC006",
        )
...
def proof_matches_checkpoint(
    ...
    if normalized_proof["checkpoint"] != _checkpoint_reference(normalized_checkpoint):
        raise AuditTrustReceiverCheckpointError(
            "receiver proof checkpoint reference does not match the supplied checkpoint",
            rule_id="ARC007",
        )
    return normalized_proof


def proof_matches_handoff(proof: dict[str, Any], handoff: Path) -> dict[str, Any]:
    ...
    try:
        verified = verify_bundle(
            Path(handoff),
            expected_bundle_id=evidence["handoff_bundle_id"],
    ...
    try:
        actual = _evidence_from_verified(verified)
    except AuditTrustReceiverError as exc:
        raise AuditTrustReceiverCheckpointError(str(exc), rule_id="ARC008") from exc
    if verified.get("bundle_type") != expected_type or actual != evidence:
        raise AuditTrustReceiverCheckpointError(
            "verified handoff does not match the authenticated receiver entry",
            rule_id="ARC008",
        )
    return verified
...
def lineage(left: dict[str, Any], right: dict[str, Any]) -> dict[str, Any]:
    ...
    if relation == "rollback":
        violations.append(
            {"rule_id": "ARC010", "message": "right receiver state is older than left"}
        )
    elif relation == "fork":
        violations.append(
            {
                "rule_id": "ARC011",
                "message": "receiver states diverge after a common prefix",
            }
        )
    ...
    core = {
        ...
        "common": {
            ...
            "handoff_bundle_id": (
                common_entry["evidence"]["handoff_bundle_id"] if common_entry else None
            ),
        },
...
def _load_canonical(
    ...
    if path.is_symlink() or not path.is_file():
        raise AuditTrustReceiverCheckpointError(
            f"{label} must be a regular non-symlink file", rule_id="ARC001"
        )
    raw = path.read_bytes()
    if not raw or len(raw) > limit:
        raise AuditTrustReceiverCheckpointError(
            f"{label} size is outside the reviewed boundary", rule_id="ARC002"
        )
    try:
        value = json.loads(
            raw.decode("utf-8"),
        ...
        raise AuditTrustReceiverCheckpointError(
            f"{label} is not strict JSON: {exc}", rule_id="ARC002"
        ) from exc
    normalized = validator(value)
    if raw != canonical_json(normalized):
        raise AuditTrustReceiverCheckpointError(
            f"{label} is not canonically serialized", rule_id="ARC002"
        )
    return normalized
...
def load_checkpoint(path: Path) -> dict[str, Any]:
    return _load_canonical(
        path, validate_checkpoint, "receiver checkpoint", MAX_CHECKPOINT_BYTES
    )


def load_proof(path: Path) -> dict[str, Any]:
    return _load_canonical(
        path, validate_proof, "receiver inclusion proof", MAX_PROOF_BYTES
    )
...
def _safe_parent(path: Path) -> Path:
    ...
    if cursor.is_symlink() or not cursor.is_dir():
        raise AuditTrustReceiverCheckpointError(
            "receiver checkpoint output parent must be a regular non-symlink directory",
            rule_id="ARC001",
        )
...
def _write_new(path: Path, payload: dict[str, Any], label: str) -> None:
    ...
    if path.is_symlink() or path.exists():
        raise AuditTrustReceiverCheckpointError(
            f"refusing to overwrite existing {label}", rule_id="ARC009"
        )
    ...
    try:
        ...
        if not stat.S_ISREG(output_stat.st_mode):
            raise AuditTrustReceiverCheckpointError(
                f"created {label} is not a regular file", rule_id="ARC001"
            )
        try:
...
def _emit(payload: dict[str, Any], output_format: str, *, stream: Any = None) -> None:
    ...
    if output_format == "json":
        ...
        return
    for key in (
        "valid", "created", "checkpoint_id", "proof_id", "state_id",
        "lineage_id", "accepted", "relation", "rule_id", "error",
    ):
        if key in payload:
    ...
    if isinstance(head, dict):
        print(
            ...
            f"handoff_bundle_id={head['handoff_bundle_id']}",
            file=stream,
        )
    ...
    if isinstance(entry, dict):
        print(
            ...
            f"handoff_bundle_id={entry['evidence']['handoff_bundle_id']}",
            file=stream,
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    selector.add_argument("--handoff-bundle-id")
    ...
    verify_proof.add_argument("--handoff", type=Path)
    verify_proof.add_argument("--format", choices=("json", "text"), default="json")

    lineage_parser = commands.add_parser("lineage")
    ...
    try:
        if args.command == "create":
            ...
            if state["state_id"] != expected_state_id:
                raise AuditTrustReceiverCheckpointError(
                    "receiver state does not match the externally pinned state id",
                    rule_id="ARC003",
                )
            checkpoint = create_checkpoint(state)
            _write_new(args.output, checkpoint, "receiver checkpoint")
            _emit(
                {
        ...
        if args.command == "verify":
            ...
            expected_checkpoint_id = _pin(
                args.expected_checkpoint_id, "expected receiver checkpoint id"
            )
            if checkpoint["checkpoint_id"] != expected_checkpoint_id:
                raise AuditTrustReceiverCheckpointError(
                    "receiver checkpoint does not match the external checkpoint pin",
                    rule_id="ARC003",
                )
            if (args.state is None) != (args.expected_state_id is None):
                raise AuditTrustReceiverCheckpointError(
                    "receiver state verification requires --state and --expected-state-id"
                )
        ...
        if args.command == "prove":
            ...
            proof = create_proof(
                ...
                handoff_bundle_id=args.handoff_bundle_id,
            )
            _write_new(args.output, proof, "receiver inclusion proof")
            _emit(
                {
                    "created": str(args.output),
        ...
        if args.command == "verify-proof":
            ...
            if args.handoff is not None:
                proof_matches_handoff(proof, args.handoff)
        ...
        if left["state_id"] != _pin(
            args.expected_left_state_id, "expected left receiver state id"
        ):
            raise AuditTrustReceiverCheckpointError(
                "left receiver state does not match the external state pin",
                rule_id="ARC003",
            )
        if right["state_id"] != _pin(
            args.expected_right_state_id, "expected right receiver state id"
        ):
            raise AuditTrustReceiverCheckpointError(
                "right receiver state does not match the external state pin",
                rule_id="ARC003",
            )
        report = lineage(left, right)
        _emit(report, args.format)
        return 0 if report["accepted"] else 1
    except (AuditTrustReceiverError, AuditTrustReceiverCheckpointError, OSError) as exc:
        error = (
            exc
            if isinstance(exc, AuditTrustReceiverCheckpointError)
            else AuditTrustReceiverCheckpointError(str(exc))
        )
        _emit(
            {"valid": False, "rule_id": error.rule_id, "error": str(error)},
