...
class ReceiverAcceptanceTrustTests(unittest.TestCase):
    ...
    def test_previous_nested_receiver_head_must_match(self):
        verified = verified_transition()
    ...
    def test_duplicate_bundle_is_rejected(self):
        state = self.anchor()
        verified = verified_transition()
        verified["bundle_id"] = state["head"]["handoff_bundle_id"]
        with self.assertRaises(AuditTrustReceiverAcceptanceTrustError):
    ...
    def test_transition_delta_must_match(self):
        verified = verified_transition()
    ...
    def test_report_identity_and_evidence_must_match(self):
        verified = verified_snapshot()
        item = report(verified)
