...
class ReceiverAdmissionTests(unittest.TestCase):
    ...
    def test_06_transition_bundle_is_admitted_with_deltas(self):
        with tempfile.TemporaryDirectory() as temporary:
    ...
    def test_17_transition_relation_policy_denies_unallowed_relation(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch(
                "agent_audit_trust_receiver_admission.verify_bundle",
    ...
    def test_19_manifest_identity_change_and_policy_inside_bundle_fail_invalid(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            with mock.patch(
                "agent_audit_trust_receiver_admission.load_manifest",
    ...
    def test_20_cli_init_validate_admit_deny_and_invalid_exit_codes(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            report = json.loads(admitted.getvalue())
            self.assertTrue(report["admitted"])
            policy = default_policy()
            policy["bundle"]["allowed_types"] = ["transition"]
