...
from agent_audit_trust_receiver_acceptance_checkpoint import (
    AuditTrustReceiverAcceptanceCheckpointError,
    _core,
...
from test_audit_trust_receiver_acceptance import (
    candidate,
    h,
    report,
    verified_snapshot,
...
class AcceptanceCheckpointTests(unittest.TestCase):
    ...
    def test_checkpoint_rejects_rehashed_payload_tamper(self):
        ...
        with self.assertRaises(AuditTrustReceiverAcceptanceCheckpointError):
            validate_checkpoint(checkpoint)

    def test_checkpoint_must_match_exact_state(self):
        states = history()[0]
        with self.assertRaises(AuditTrustReceiverAcceptanceCheckpointError) as raised:
            checkpoint_matches_state(create_checkpoint(states[0]), states[1])
        self.assertEqual("ASC004", raised.exception.rule_id)
    ...
    def test_proof_selects_by_receiver_bundle_id(self):
        state = history()[0][2]
    ...
    def test_proof_requires_exactly_one_selector(self):
        ...
        with self.assertRaises(AuditTrustReceiverAcceptanceCheckpointError) as none:
            create_proof(state, checkpoint)
        with self.assertRaises(AuditTrustReceiverAcceptanceCheckpointError) as both:
            create_proof(state, checkpoint, sequence=1, handoff_bundle_id=state["head"]["handoff_bundle_id"])
        self.assertEqual("ASC005", none.exception.rule_id)
        self.assertEqual("ASC005", both.exception.rule_id)

    def test_missing_bundle_selector_is_rejected(self):
        state = history()[0][0]
        with self.assertRaises(AuditTrustReceiverAcceptanceCheckpointError) as raised:
            create_proof(state, create_checkpoint(state), handoff_bundle_id="f" * 64)
        self.assertEqual("ASC005", raised.exception.rule_id)
    ...
    def test_rehashed_audit_path_tamper_is_rejected(self):
        ...
        self.assertEqual("ASC006", raised.exception.rule_id)
    ...
    def test_checkpoint_substitution_is_rejected(self):
        ...
        self.assertEqual("ASC007", raised.exception.rule_id)

    def test_snapshot_receiver_bundle_binding(self):
    ...
    def test_transition_receiver_bundle_binding(self):
        states, verified = history()
    ...
    def test_receiver_bundle_substitution_is_rejected(self):
        states, verified = history()
        proof = create_proof(states[0], create_checkpoint(states[0]), sequence=1)
        with mock.patch.object(_core, "verify_bundle", return_value=verified[1]):
            with self.assertRaises(AuditTrustReceiverAcceptanceCheckpointError) as raised:
                proof_matches_handoff(proof, Path("wrong-bundle"))
        self.assertEqual("ASC008", raised.exception.rule_id)
    ...
    def test_immutable_output_is_mode_0600_and_no_overwrite(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            checkpoint = create_checkpoint(history()[0][0])
            _core._write_new(output, checkpoint, "acceptance checkpoint")
            self.assertEqual(0o600, os.stat(output).st_mode & 0o777)
            before = output.read_bytes()
            with self.assertRaises(AuditTrustReceiverAcceptanceCheckpointError) as raised:
                _core._write_new(output, checkpoint, "acceptance checkpoint")
            self.assertEqual(before, output.read_bytes())
        self.assertEqual("ASC009", raised.exception.rule_id)
    ...
    def test_lineage_same_descendant_rollback_and_fork(self):
        ...
        self.assertEqual("ASC010", rollback["violations"][0]["rule_id"])
        fork_verified = third_transition(verified[0]["candidate"])
        fork_verified["bundle_id"] = h(991)
        fork_verified["candidate"] = candidate(900, 2, 2, 9, 2)
        fork_verified["head_handoff_bundle_id"] = fork_verified["candidate"]["head"]["handoff_bundle_id"]
        fork = append_transition(states[0], report(fork_verified), fork_verified)
        result = lineage(states[1], fork)
        self.assertEqual("ASC011", result["violations"][0]["rule_id"])
    ...
    def test_cli_create_prove_verify_and_lineage(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with contextlib.redirect_stdout(io.StringIO()):
                self.assertEqual(0, main(["create", str(right), str(checkpoint_path), "--expected-state-id", states[1]["state_id"]]))
            checkpoint = load_checkpoint(checkpoint_path)
            with contextlib.redirect_stdout(io.StringIO()):
                self.assertEqual(0, main(["prove", str(right), str(checkpoint_path), str(proof_path), "--expected-state-id", states[1]["state_id"], "--expected-checkpoint-id", checkpoint["checkpoint_id"], "--handoff-bundle-id", states[1]["head"]["handoff_bundle_id"]]))
                self.assertEqual(0, main(["verify-proof", str(proof_path), str(checkpoint_path), "--expected-checkpoint-id", checkpoint["checkpoint_id"]]))
                self.assertEqual(0, main(["lineage", str(left), str(right), "--expected-left-state-id", states[0]["state_id"], "--expected-right-state-id", states[1]["state_id"]]))
