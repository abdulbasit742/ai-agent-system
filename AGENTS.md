...
- Never commit secrets, populated `.env` files, private data, generated reports, audit logs, segment archives, segment catalogs, catalog checkpoints, catalog inclusion proofs, catalog consistency proofs, audit evidence bundles, generated admission policies, admission decisions, audit bundle trust states, audit trust checkpoints/proofs, audit trust consistency proofs, or trust-state lock files.
...
- The public wheel must remain runtime-dependency-free and contain only the reviewed twenty-module allowlist.
- Tests, docs, action internals, integration locks, external repositories, environment files, baselines, reports, audit-log data, segment archives, segment catalogs, catalog checkpoints, catalog proofs, catalog consistency proofs, audit evidence bundles, admission policies, admission decisions, audit bundle trust states, audit trust checkpoints/proofs, audit trust consistency proofs, and lock files must never enter the wheel.
...
- Audit evidence bundle creation must verify every checkpoint, inclusion proof, optional consistency proof, and optional sealed segment before creating output. Invalid source evidence must never be copied into a bundle.
- Snapshot bundles require one pinned candidate checkpoint and one or more candidate inclusion proofs. Transition bundles additionally require the pinned previous checkpoint and one consistency proof that binds both checkpoints.
- Bundle verification must require externally retained `bundle_id` and candidate `checkpoint_id`; transition verification must also require the previous checkpoint pin. Self-consistent bundle files alone do not prove freshness.
- Bundle manifests and `SHA256SUMS` must use canonical ordering, safe relative paths, exact file roles, digests, sizes, and an exact file boundary. Extra, missing, renamed, duplicate, symlinked, or modified files must fail with stable `AUBxxx` diagnostics.
- Bundles must be new immutable directories published only after complete staging verification. Never overwrite an existing output, infer missing proofs, merge forks, or use a compressed archive that introduces extraction-path risk.
- Selected sealed segments may be copied only after proof-to-directory verification. Proof-only bundles may omit segment bytes and remain independently verifiable from the pinned checkpoint.
- Audit evidence bundles are unsigned integrity and portability containers. They do not authenticate the producer, establish witness consensus, or replace external freshness pins.
- Audit bundle admission policies are consumer-owned inputs and must never be loaded from inside the bundle being evaluated.
- Audit bundle admission must verify the complete bundle and exact external bundle/checkpoint pins before applying authorization policy. Invalid evidence is not an ordinary policy denial.
- Admission policies must use exact versioned schemas, strict types, canonical hashing, bounded counts, sorted unique allowlists, and stable `AUA001` through `AUA016` controls.
...
- Audit trust checkpoints may be created only from a fully validated state bound to an externally retained `state_id`; stored head summaries alone are insufficient.
- Audit trust checkpoint leaves and nodes must retain separate `0x00` and `0x01` SHA-256 domains and RFC 6962 largest-power-of-two splitting. Never duplicate odd leaves.
- Trust inclusion-proof verification must require an externally retained checkpoint ID, reconstruct the exact root, reject missing/extra siblings, and optionally fully re-verify the authenticated portable bundle.
- Trust checkpoint/proof outputs must be canonical, bounded, immutable, symlink-safe, mode-0600, and atomic no-overwrite files. Proof-only verification may omit the state but never the checkpoint pin.
- Audit trust lineage accepts only identical or right-descendant states and must retain stable `ATC010` rollback and `ATC011` fork denials. Never merge forks automatically.
- Audit trust checkpoints and proofs are unsigned integrity commitments, not signer authentication, witness consensus, trusted timestamps, or public transparency-log publication.
- Audit trust consistency proof creation must validate both complete states and exact externally pinned checkpoints, and may accept only identical or right-descendant histories.
- Audit trust consistency proofs must reconstruct both checkpoint roots from canonical maximal aligned power-of-two frontiers; a matching `consistency_id` alone is insufficient.
- Every descendant proof must expose the first appended transition as a one-leaf frontier segment and bind its previous entry hash, checkpoint, catalog, and generation delta to the retained head.
- Rollback and fork requests must retain stable `ATK009` and `ATK010` denials; malformed transition boundaries must retain `ATK011`. Denied creation must not create proof files.
- Audit trust consistency outputs must remain canonical, bounded, mode-0600, immutable, symlink-safe, atomic no-overwrite files and must exclude audit records, source contents, credentials, and signing material.
- Audit trust consistency IDs are unsigned integrity commitments, not signer authentication, witness consensus, trusted timestamps, or public transparency-log publication.
...
- Ordinary CI may build, verify, admit, compare, exercise temporary audit logs, typed-event gates, segment rotations, segment catalogs, catalog checkpoints, catalog inclusion and consistency proofs, portable audit evidence bundles, audit bundle admission policies, audit bundle trust states, audit trust checkpoints/proofs/consistency proofs, release trust states, release checkpoints, and release consistency proofs, and upload evidence decisions but must never publish a package, create a release, request OIDC credentials, use signing keys, or read registry secrets.

Verification:

```bash
python -m unittest discover -s tests -v
python -m compileall -q agent_audit.py agent_audit_events.py agent_audit_segments.py agent_audit_catalog.py agent_audit_checkpoint.py agent_audit_consistency.py agent_audit_bundle.py agent_audit_admission.py agent_audit_trust.py agent_audit_trust_checkpoint.py agent_audit_trust_consistency.py agent_system.py agent_system_legacy.py agent_policy.py agent_config.py agent_baseline.py agent_git.py agent_changed_lines.py agent_cli.py agent_version.py tests scripts
...
python -m unittest discover -s tests -p "test_audit_trust_checkpoint.py" -v
python -m unittest discover -s tests -p "test_audit_trust_consistency.py" -v
