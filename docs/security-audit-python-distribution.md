...
Task 6 introduced the installable wheel and console scripts. Tasks 14 through 24 expand the reviewed runtime boundary for strict audit integrity, typed event admission, segment rotation, canonical catalogs, portable catalog checkpoints, compact catalog consistency proofs, portable audit evidence bundles, consumer-owned bundle admission, pinned audit bundle trust states, portable audit trust checkpoints, and compact audit trust consistency proofs while keeping the package dependency-free.
...
`scripts/validate_wheel.py` enforces an exact twenty-module allowlist:
...
- tests, action metadata, integration locks, environment files, audit-log data, segment archives, catalogs, checkpoints, inclusion/consistency proofs, audit evidence bundles, admission policies, admission decisions, audit trust states, trust checkpoints/proofs, trust consistency proofs, lock files, baselines, and generated reports
...
- `basit-agent-audit-trust-consistency`
- `agent-system`
- `agent-changed-lines`
...
Audit runtime code is included, but audit JSON Lines files, segment directories, manifests, catalog files, checkpoint files, inclusion/consistency proof files, audit bundle directories, admission policies, admission decisions, trust-state files, trust checkpoint/proof files, trust consistency proof files, lock sidecars, recovery copies, reports, and CI evidence are not package source. Raw paths, command arrays, and Git refs are normalized to domain-separated references before new audit records are stored.

Segment manifests, catalog entries, checkpoints, proofs, bundle manifests, admission reports, trust-state entries, trust checkpoint/proof files, and trust consistency proofs contain only bounded identities, counts, roles, algorithms, hashes, generations, compact frontiers, and stable diagnostics. Installed commands create runtime evidence only in caller-selected locations.
...
CI builds a wheel on Python 3.11 and 3.12, validates the exact archive and script boundary, installs it into an isolated virtual environment without dependencies, and executes from outside the source checkout. Dedicated package workflows exercise all twenty-two console aliases across the reviewed features. The audit trust consistency package smoke creates retained and candidate checkpoints, creates compact proof evidence, removes both complete states, and verifies the proof through both installed aliases.
