...
`scripts/validate_wheel.py` enforces an exact twenty-five-module allowlist:
...
- tests, action metadata, integration locks, environment files, audit-log data, segment archives, catalogs, checkpoints, inclusion/consistency proofs, evidence or trust handoff bundles, admission policies, decisions, receiver states, receiver checkpoints/proofs, lock files, baselines, and generated reports;
- wrong project name, version, Python requirement, or console entry points.

## Installed command boundary

The exact reviewed command set contains thirty aliases:
...
- `basit-agent-audit-trust-receiver`, `basit-agent-audit-trust-receiver-checkpoint`;
- the corresponding fifteen `agent-*` compatibility aliases.
...
Audit runtime code is included, but audit JSON Lines files, segment directories, manifests, catalog files, checkpoint files, inclusion/consistency proof files, evidence bundle directories, trust handoff directories, admission policies, decisions, trust-state files, receiver-state files, receiver checkpoint/proof files, lock sidecars, recovery copies, reports, and CI evidence are not package source. Raw paths, command arrays, and Git refs are normalized to domain-separated references before new audit records are stored.

Trust handoff manifests, admission decisions, receiver states, checkpoints, and proofs contain only bounded checkpoint references, authenticated entry evidence, exact roles, sizes, digests, policy hashes, decision IDs, counts, Merkle roots, and stable diagnostics. The package does not contain generated evidence, consumer policies, decisions, or externally retained freshness pins.
...
CI builds a wheel on Python 3.11 and 3.12, validates the exact archive and script boundary, installs it into an isolated virtual environment without dependencies, and executes from outside the source checkout. Dedicated package workflows exercise all thirty console aliases. The receiver-checkpoint package smoke creates actual snapshot and transition handoffs, advances a receiver state, creates a receiver checkpoint and head proof through both installed aliases, deletes the complete state, and independently re-verifies the proof against the portable transition handoff.
