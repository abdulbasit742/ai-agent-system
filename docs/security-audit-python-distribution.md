...
`scripts/validate_wheel.py` enforces an exact twenty-six-module allowlist:
...
- tests, action metadata, integration locks, environment files, audit-log data, segment archives, catalogs, checkpoints, inclusion/consistency proofs, evidence or trust handoff bundles, admission policies, decisions, receiver states, receiver checkpoints/proofs, receiver consistency proofs, lock files, baselines, and generated reports;
- wrong project name, version, Python requirement, or console entry points.

## Installed command boundary

The exact reviewed command set contains thirty-two aliases:
...
- `basit-agent-audit-trust-receiver`, `basit-agent-audit-trust-receiver-checkpoint`, `basit-agent-audit-trust-receiver-consistency`;
- the corresponding sixteen `agent-*` compatibility aliases.
...
Audit runtime code is included, but audit JSON Lines files, segment directories, manifests, catalog files, checkpoint files, inclusion/consistency proof files, evidence bundle directories, trust handoff directories, admission policies, decisions, trust-state files, receiver-state files, receiver checkpoint/proof files, receiver consistency proof files, lock sidecars, recovery copies, reports, and CI evidence are not package source. Raw paths, command arrays, and Git refs are normalized to domain-separated references before new audit records are stored.
...
CI builds a wheel on Python 3.11 and 3.12, validates the exact archive and script boundary, installs it into an isolated virtual environment without dependencies, and executes from outside the source checkout. Dedicated package workflows exercise all thirty-two console aliases. The receiver-consistency package smoke creates retained and candidate receiver states and checkpoints, creates a compact proof through one installed alias, removes both complete states, and verifies the proof through the compatibility alias using only externally pinned checkpoints.
