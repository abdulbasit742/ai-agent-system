...
Task 6 introduced the installable wheel and console scripts. Tasks 14 through 25 expand the reviewed runtime boundary for strict audit integrity, typed event admission, segment rotation, canonical catalogs, portable catalog checkpoints, compact catalog consistency proofs, portable audit evidence bundles, consumer-owned bundle admission, pinned audit bundle trust states, portable audit trust checkpoints, compact audit trust consistency proofs, and portable trust handoff bundles while keeping the package dependency-free.
...
`scripts/validate_wheel.py` enforces an exact twenty-two-module allowlist:
...
The exact reviewed command set contains twenty-four aliases:
...
- `basit-agent-audit-trust-checkpoint`, `basit-agent-audit-trust-consistency`, `basit-agent-audit-trust-bundle`;
- the corresponding twelve `agent-*` compatibility aliases.
...
Trust handoff manifests contain only bounded checkpoint references, selected proof identities, exact roles, sizes, digests, and stable diagnostics. The package does not contain generated handoff evidence or externally retained freshness pins.
...
CI builds a wheel on Python 3.11 and 3.12, validates the exact archive and script boundary, installs it into an isolated virtual environment without dependencies, and executes from outside the source checkout. Dedicated package workflows exercise all twenty-four console aliases. The audit trust handoff package smoke creates retained/candidate checkpoints and consistency/head proofs, packages them through the `basit-*` alias, removes all loose evidence, and verifies the handoff through the `agent-*` alias.
